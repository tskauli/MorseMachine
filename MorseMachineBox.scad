...
// mod LA4ZCA:
// -button 1.3 mm higher
// -hole in front wall for volume control
// -larger internal wire hole
// -adjusted trimpot dimensions
// -holes for speed pot and speaker in top

part = "all"; // [ top, bottom, button, buttons, all ]
with_battery = 0; // [ 1:true, 0:false ] 
extra_room = 40; //20.6; // size of extra room, if any, set to 0 otherwise
...
button_dx1 = 15.2 - 6.0;
button_dx2 = 23.2 - 6.0;
trimpot = [10,5];//[9.5, 4.4]; // trimpot size, no margin
trimpotdx = -(pcb[0]/2  - 3.9 - 9.5/2);
trimpotdy = pcb[1]/2 - 0.9 - 4.4/2; 
trimmer_dia = 2.8; // 2.3 with no margin
trimmer = [-(pcb[0]/2-4.4-trimmer_dia/2), pcb[1]/2-0.7-trimmer_dia/2, 0]; // coordinates of trimmer hole (was -4.8)
...
module bottom() {
	...
	module sub() {
        ...
        if (extra_room > 0) translate([pcb[0]/2+tol+wall+extra_room/2, 0, wall]) {
            translate([0, pcb[1]/2 - 12.0/2, pcb2floor*.8]) rotate([0, -90, 0]) cylinder(r=12.0/2, h=extra_room); // hole in internal wall
            for (dx = [-6.0, 6.0]) translate([dx, pcb[1]/2 - 6.25/2, pcb2floor*.5]) rotate([0, -90, -90]) cylinder(r=6.25/2, h=extra_room, $fn=24); // holes in external wall
            
        // hole for volume control in front side wall, +1 mm above plug holes
            translate([0, -pcb[1]/2 - 7.0/2, pcb2floor*.5+1]) rotate([0, -90, -90]) cylinder(r=6.25/2, h=extra_room, $fn=24); 
...
module top() {
	...
	module sub() {
       ...
       if (extra_room > 0) translate([pcb[0]/2+tol+wall+extra_room/2, 0, -d]) {
            ...
            translate([-5,-10,0]) cylinder(d=7,h=20); // hole for potentiometer
           
           // speaker holes
           translate([-5,15,0])
           for (a=[0:45:360]){ translate([6*sin(a),6*cos(a),0])cylinder(d=2,h=20);};
...
if (part=="top" || part=="all") translate([-0, 35, pcb2roof+0*tol+wall]) rotate([180,0,0]) top();
